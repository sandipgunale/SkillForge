...
| 91 | /review | MINOR batch: press release clears transform (clearProps onComplete) so CSS hover scale regains control after the first press; useNumeralRoll gains `visible` flag (LearningPathStats score re-rolls when its span mounts); QuizProgress initial text "0" kills the one-frame final-value flash; Mastery proxy tween re-arms on sceneReady (lazy-mount race; wrap visibility kept per row 74 contract — hidden under reduced motion, static cap is the fallback); ForgeSignature rAF truly pauses off-screen/tab-hidden (IO + visibilitychange); Engine rect width via style (calc invalid in SVG attribute grammar) | Minor batch | P2 | AICenter/AICopilot/QuizProgress/LearningPathStats/MasterySection/ForgeSignature/EngineSection | — |
...
| 93 | /review | Fix landing + verification: ForgeSignature draw loop re-schedule restored (the /review rewrite dropped `raf = requestAnimationFrame(draw)` — canvas painted once at scattered state, scrub had no effect; re-fixed in-place); all 8 probes green — signature (converged span 0.74, reduced entry/dissolve/converged), micro-hooks (border trace, press clearProps), landing-hero (beat order + pre-hide visibility contract via MutationObserver), hero-reduced, landing-entrance, identity-check, mastery-rotate, app-motion (full + reduced) | Fix landing | P1 | commit 5e78738; lint 0/0, build clean (2.36s) | — |
| 94 | /review | Lint regression caught during fixes: useNumeralRoll `visible` deps edit landed in useMountAnimation (no-undef to/pad/suffix/visible + unused deps) — corrected deps arrays (useMountAnimation: ref,reduced,y,scale,scaleX,opacity,blur,height,duration,ease,delay,onComplete,props,deps; useNumeralRoll: +visible) and dropped two dead eslint-disable directives | Nits | P3 | Review-fix batch only — verified by lint 0/0 | — |
| 95 | /cso | Security Posture Report (docs/SECURITY-REPORT-motion.md): PASS — 0 High/Med/Low, 2 Info. Git history (90 commits) + source tree secret scans clean (3 benign hits: JwtServiceTest TEST_SECRET fixture, showcase demo JWT strings, lockfile integrity hashes); npm audit 0 vulns; CI/compose env-var injection clean; OWASP client-side sweep clean (no HTML sinks, no dynamic URLs, no canvas readback, self-painted canvases only). Hardening backlog: npm audit + Dependabot in CI, SHA-pin actions, markdown URL scheme allowlist, data-pad clamp | Info x2 | P3 | docs/SECURITY-REPORT-motion.md | — |
| 96 | /cso | Branch surface confirmed no new input paths: all dynamic values are geometry/pointer/scroll math; changed files touch no storage/network/auth code | Info | — | Diff scope: 30 source files + 2 docs | — |
| 97 | /qa | QA report (docs/QA-REPORT-motion.md): 13 probes green, 0 app bugs. Battery: signature (two-act + converge span 0.74), micro-hooks (border trace + press clearProps), landing-hero (beat order + pre-hide contract via MutationObserver + halo rest), hero-reduced, landing-entrance, identity-check, mastery-rotate, app-motion (ornament/stats/gamification/AI press/morph, full+reduced), landing-mobile (no overflow), + 3 new probes: cap-return (scroll-back re-forms animated, no snap), tab-order (hidden CTAs out of tab order pre-entrance, reachable post, container-level contract under reduced), theme-morph (light→dark gate 600ms full / 1e-05s reduced) | Bugs: 0 | P1 | 4 probe-harness fixes (wrong selectors/assertions), 0 app changes; e2e-admin blocked (no seeded admin) | — |
| 98 | /benchmark | Benchmark report (docs/BENCHMARK-REPORT-motion.md): PASS — bundle +12 kB raw / +3 kB gzip (+0.4%, 77→78 chunks), vendor-motion −7 kB (dead PRESETS removal), vendor-three unchanged (lazy 3D intact); load probe: TTFB 2ms, domComplete 189ms both, LCP 268→280ms (noise), CLS 0.00033 both, INP 149→139ms (improved), transfer 772 kB both, 23 requests both | No regression | P1 | Base built from worktree of 3b601ba (v0.3.0.0); all CWV budgets met with 5-10x headroom | — |
| 99 | /benchmark | Methodology note: npm ci/robocopy/child-spawning commands get killed by the harness ~2min — base built via node_modules copy (lockfiles differ in metadata only, dep sets identical); perf probe runs single-pass to stay under the cap | Info | — | Operational; multi-pass averages deferred to release-time Lighthouse | — |
| 100 | user-report | Dev server was down (page stale/HMR-frozen); restarted on 5173, fresh-load probe 10/10: theme toggle visible (landing + auth), login/register cards visible, 0 console errors. Probe also caught real bug: FloatingField dropped the `type` prop → email inputs rendered as type=text (no email keyboard/autofill). Fixed + verified (commit 83cc172) | Bug: 1 | P1 | Environment issue (dead server), not app state; user WIP uncommitted: AppLogo logo swap + index.css de-@apply (lint: unused Flame import) | — |
| 101 | user-request | Brand pass: new shared LogoMark component (theme-aware img pair, alt="" decorative) replacing the lucide Flame icon in AppLogo (lint fix — unused import), AuthLogo, EmptyState default, MissionOverview; asset skillforge_logo_Dark.png committed; lint 0/0, build clean, probes: imgs decode 1536x1024, visible per pair, no .lucide-flame anywhere (commit 2171cf5) | 0 bugs | P1 | index.css de-@apply WIP left uncommitted (user's separate change, verified working via dev server) | — |
| 102 | user-report | Landing cap invisible until click under prefers-reduced-motion (or offscreen demand frameloop): entrance spring starts at scale 0.0001 and the demand loop only advances on invalidate (pointer/resize) — cap frozen at scale ~0 until interaction. Fix: settle spring to scale 1 on the reduced-motion frame (static pose); full-motion path byte-identical. Fixed a TDZ bug in the first attempt (referenced `spring` const before its declaration — caught by adding pageerror capture to probe). Verified: reduced-motion probe 5/5 (cap 100% painted with NO click, 0 page errors, auth cards visible reduced + full). Auth card visibility re-confirmed on fresh load (gsap.from + reduced paths safe). Commits: 2ed05b5 | Bug: 1 | P1 | Dev server dies between harness commands (Ctrl+C on job group) — run `npm run dev` in your own terminal for a persistent server | — |
| 103 | prod-audit | Theme pre-hydration: next-themes (React/Vite, non-Next) does NOT inject its bootstrap script, so `<html>` had no `dark` class until React mounted → wrong-theme first paint (flash) + nondeterministic first render on dark-pref systems. Fix: inline bootstrap script in index.html (storage key `theme`, class attr, system fallback, sets colorScheme). Verified: dark-pref context paints `html.dark` at load, toggle aria reflects System (accurate), switch→Dark persists across reload | Bug: 1 | P1 | index.html; matches ThemeProvider config (attribute=class, defaultTheme=system, storageKey default) | — |
| 104 | prod-audit | Auth pages gated by unbounded session bootstrap: PublicRoute shows PageLoader while `bootstrapping`; `bootstrap()` awaited `/auth/refresh` on the 120 000ms axios timeout → with a stale localStorage session + slow/dead backend the login/register card was blocked up to 120s (appears "not rendered"). Fix: `/auth/refresh` call carries `timeout: 5000` (bounds both bootstrap and the 401-retry path); on timeout the session is cleared and the form renders. Verified: stale user + hanging backend (accepts request, never responds) → card appears in 5.7s | Bug: 1 | P1 | authStore bootstrap + interceptors.refreshAccessToken; forms always visible, redirect-on-success preserved | — |
| 105 | prod-audit | Decorative WebGL not isolated: AuthLayout layer-3 wrapper + LivingCoreScene wrapper lacked `pointer-events-none` → the full-viewport R3F canvas was an interactive event root on the auth screen (violates the "WebGL is decorative, never blocks UI" contract). Fix: `pointer-events-none` on both wrappers. Verified: canvas wrapper computed pointer-events none; elementFromPoint over the email input returns the INPUT | Bug: 1 | P1 | Matches GraduationCapScene + KnowledgeConstellation pattern; cursor repulsion becomes non-interactive (decoration only) | — |
| 106 | prod-audit | LivingCoreScene kept `frameloop="always"` under prefers-reduced-motion (rendered every frame despite early-returning useFrame — wasted GPU/battery, violates "static under reduced motion"); also no WebGL-availability probe → with WebGL disabled R3F threw "Error creating WebGL context" and the plain Suspense (no boundary) let it escape to the App boundary → whole auth page white-screened instead of showing the form. Fix: frameloop `reducedMotion || tabHidden ? "demand" : "always"` (one static frame then freeze); WebGL probe returns null when unavailable; AuthLayout wraps the scene in a degrade-to-null SceneBoundary. Verified: reduced-motion auth clean, WebGL-disabled landing shows CapEmblem + auth form still renders, 0 page errors | Bug: 2 | P1 | Decorative-scene contract; auth UI now fully WebGL-independent | — |
| 107 | prod-audit | Hero cap could stay invisible on slow networks: ArrivalSection faded the scene in with a one-shot useMotionScope keyed on `sceneReady`; if the lazy chunk hadn't resolved when the effect fired, `[data-cap-scene]` was absent → fade never ran → inline `opacity:0` left the cap invisible forever. Fix: GraduationCapScene fades itself in on mount (useMountAnimation — timing-independent, reduced-safe); ArrivalSection removed the inline opacity-0 + one-shot block; Arrival + Mastery Suspense now fall back to CapEmblem (was `null` → blank swap window). Verified: throttled chunk (4s) → emblem visible at 1.5s, scene visible (opacity 1) after load | Bug: 2 | P1 | Root cause distinct from row 102 (frozen spring); this is the lazy-mount race | — |
| 108 | prod-audit | Landing and auth route groups had no `errorElement` → a ChunkLoadError on `/` or `/login` rendered a blank screen instead of AppError (MainLayout + Showcase already had one). Fix: `errorElement: <AppError />` on both groups | Bug: 1 | P2 | routes/index.jsx; robustness parity with protected/showcase groups | — |
| 109 | prod-audit | THREE.Clock deprecation warning (3 per page): three r183+ warns in the Clock constructor; @react-three/fiber 9.6.1 (and latest 9.7.0 — verified) constructs `new THREE.Clock()` per Canvas internally, so no app change can silence it. Fix: pin three to `~0.182.0` (last version before the r183 deprecation; R3F peer range `>=0.156` satisfied; all scene APIs — RoundedBoxGeometry addon, Capsule/Tube geometry, damp, lerp — unchanged). Verified: zero console warnings across every probe | Bug: 1 | P3 | Dependency-level root cause, not a warning-hide; documented in report | — |
| 110 | prod-audit | Verification: two Playwright suites (qa-stability.mjs 20/20, qa-stability2.mjs 11/11 — 31/31): theme pre-hydration + persistence, first-load no-interaction at 1440×900 + 390×844 (landing/login/register), reduced-motion (cap painted static, cards visible), pointer hit-test isolation, bootstrap bounded (<8s with hanging backend), slow-chunk cap fallback, route transitions, DPR 2 canvases, WebGL-disabled fallback + auth form, scroll CLS 0.0002 (<0.05 budget), mastery mounts on scroll, auth theme switch; lint 0/0, build clean; only remaining console noise = Chrome headless SwiftShader "GPU stall" driver hints (no app code) | Pass | P1 | 20/20 + 11/11; no app changes required by probes | — |
| 111 | cap-rebuild | GraduationCapScene rebuilt from scratch per the cap-rebuild spec (user directive; reference image unavailable to the model — written spec used). New structure: tapered LatheGeometry crown with a real circular underside opening + BackSide inner-cavity dome (actual depth, not a painted circle); gold trim ring + band around the lower rim; diamond-oriented beveled RoundedBox board (real 0.16 thickness); gold cord (TubeGeometry CatmullRom) + 14-strand tassel (single merged BufferGeometry, one draw call) with flared collar; 5 distinct materials (board fabric / crown fabric / deep interior / polished gold trim / gold cord+strands). New --cap-gold token (tokens.css + index.css, light+dark) drives the gold via the existing live-palette lerp; fabric tokens shifted blue-black. Studio lighting: key/fill/rim + gold accent point light + PMREM RoomEnvironment set via Canvas onCreated (scene.environment, intensity 0.55) so metals reflect properly; BASE_TILT -0.2 + INITIAL_YAW -0.55 present the diamond board with the underside visible from frame one; idle spin slowed (0.055, breath 0.3). CapEmblem tassel now uses --cap-gold for fallback consistency. New probe cap-rebuild-probe.mjs (9/9, sharp pixel analysis): gold trim + dark fabric presence, silhouette 75% of slot, hero title/CTA not occluded, reduced-motion static pose paints, dark-theme switch re-tints with gold still present (11.5k px), zero page errors. Regression: qa-stability 20/20 (1 probe fix — D-reduced-cap-painted now polls for settle; the fixed 3s measure raced the 0.6+0.6s reduced fade under parallel-suite CPU contention), qa-stability2 11/11; lint 0/0, build clean, cap chunk 8.8 kB (was 6.8) | Pass | P1 | User gate held: visual approval of shots/cap-rebuild/*.png against the reference | — |
